...
- 工具集覆盖：search / get / collections / tag / note / metadata / item + `import_by_identifier` / `find_missing_pdfs` / `find_related_papers` / `synthesize_annotations` / `find_duplicates` + `merge_duplicates` / `batch_update_tags` / `manage_pdf_resolvers` / `enrich_item_metadata` / `find_doi` 等
