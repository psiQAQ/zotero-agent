...
| 真实 Starter API | Passed（面板测试连接 1 次 + MCP 最小查询 1 次；本地计数 2/50） |
...
真实 MCP 验证使用 `TS=("graph neural network") AND PY=(2025-2026)` 且 `maxResults: 1`：Starter API 报告 8,768 条命中，插件返回 1 条规范化记录并使用 1 次请求。运行态回读确认所选 plan 为 Free Trial、当日本地计数为 2/50，API Key 和 `X-ApiKey` header 均未出现在 MCP 响应中。
