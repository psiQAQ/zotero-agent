...
execSync(
  "npx tsc src/modules/authGuard.ts src/modules/evalTool.ts src/modules/mcpProtocol.ts src/modules/httpByteReader.ts src/modules/semantic/hybridSearch.ts src/modules/pdfResolvers.ts src/modules/pdfIdentifier.ts src/modules/titleSimilarity.ts src/modules/importDedup.ts src/modules/metadataMerge.ts src/modules/scihubSources.ts src/modules/scihubProxy.ts --outDir .tmp-test --module commonjs --target es2022 --skipLibCheck --moduleResolution node",
  { stdio: "inherit" },
);
