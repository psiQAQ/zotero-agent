...
它提供 **47 个工具**，覆盖文献检索、Web of Science Starter 检索、元数据补全、按标识符导入（DOI / arXiv / ISBN / PMID）、批量文献导入（BibTeX / RIS / CSL-JSON）、预印本升级正式版、DOI 修复、灰色源 PDF 下载（Sci-Hub / Anna's Archive）、查重与合并、批量打标签、引文图扩展、注释综述、伴生插件桥接（茉莉花、Linter）——外加一个兜底的 `run_javascript`，可在 Zotero 进程内执行内置工具之外的任意自动化。
...
可选的 `search_web_of_science` 工具通过 Clarivate Starter API 执行 Web of Science 高级检索并返回规范化题录。它使用用户自己的 API Key，默认关闭，并按所选 plan 串行限速、保守记录本地日用量、限制单次结果数。结果带 DOI、PMID 或 ISBN 时可继续交给现有 `import_by_identifier`。

非高校人员可以申请 Free Trial；机构 plan 另有订阅和审批条件。申请资格、API Key 获取、plan 限制、安全说明和本地额度统计边界见 [Web of Science Starter API 中文指南](./docs/web-of-science-zh.md)。
...
| `search_web_of_science` | 用高级检索式查询 Clarivate Web of Science Starter API；按 plan 限速、限制日用量和单次结果数，并返回可衔接标识符导入的规范化记录。 |
| `import_by_identifier` | 通过 DOI、arXiv ID、ISBN 或 PMID 导入条目。 |
| `import_bibliography` | 批量导入 BibTeX / RIS / CSL-JSON（格式自动识别）；按 DOI / 标题相似度幂等查重；默认 dry-run 出导入计划。 |
...
| `find_doi` | 经 CrossRef 标题相似度（≥0.86 阈值）反查 DOI；`mode:"repair"` 可校验坏 DOI（Handle System API）并提议替换；默认 dry-run，confirm 写入需开启 write.enabled。 |
...
| `upgrade_preprints` | 经 OpenAlex 标题搜索查找 arXiv 类预印本的正式发表版并升级 DOI/刊名/日期/条目类型；旧值备份进 Extra；默认 dry-run。 |
...
| `fetch_chinese_metadata` | 经茉莉花（jasminum）伴生插件为顶层中文附件和知网快照抓取知网/万方/维普元数据（适用性分类 + 防挂起 watchdog）。 |
| `lint_metadata` | 经 zotero-format-metadata（Linter）对条目跑格式规则——标题大小写、日期、期刊缩写、中文姓名拆分；未知规则 id 拒绝并返回有效清单。 |
...
| 工具 | 27 | **47** —— 新增 20 个（Web of Science 检索、标识符与批量文献导入、缺 PDF 审计、引文图、查重、批量标签、元数据补全、DOI 反查与修复、预印本升级、灰色源下载、伴生插件桥接 等） |
| 搜索 | 关键词 | + RRF **混合**语义搜索、0 结果**降级级联** |
| 测试 | 无 | 31 场景进程内 **selfTest** + node 单测（91 例） |
...
| 来源 | 吸收为 | 状态 | TODO |
| --- | --- | --- | --- |
| [54yyyu/zotero-mcp](https://github.com/54yyyu/zotero-mcp) | `import_by_identifier`、`import_bibliography`（BibTeX / RIS / CSL-JSON）、`find_missing_pdfs`、`find_related_papers`（OpenAlex）、`check_retractions`（scite）、`synthesize_annotations` | ✅ | 批量 OA 索引 |
...
| 来源 | 吸收为 | 状态 | TODO |
| --- | --- | --- | --- |
| [zotero-metadata-hunter](https://github.com/federicotorrielli/zotero-metadata-hunter) | `enrich_item_metadata`（从 DOI CSL-JSON + OpenAlex 补字段）、`upgrade_preprints`（经 OpenAlex 标题搜索升级正式版） | ✅ | — |
| [zotero-doi-fix](https://github.com/pandaAIGC/zotero-doi-fix) | `find_doi`（标题相似度融合 + `mode:"repair"`：Handle System 校验 → 替换并备份）、`extract_identifier_from_pdf` | ✅ | — |
...
| 插件 | 能力 | 当前 agent 边界 | TODO（专用工具） |
| --- | --- | --- | --- |
| [jasminum 茉莉花](https://github.com/l0o0/jasminum) | 中文元数据抓取（知网 / 万方 / 维普） | ✅ 专用工具 **`fetch_chinese_metadata`**（v2.1.0）：适用性分类 + 防挂起 watchdog，驱动 `Zotero.Jasminum` 任务队列 | — |
| [zotero-updateifsE 绿青蛙](https://github.com/redleafnew/zotero-updateifsE) | 影响因子 / JCR 与中科院分区 | 经 `run_javascript` 逐条目调其 easyScholar 更新路径 | `update_journal_metrics(scope)` 工具 |
| [zotero-format-metadata](https://github.com/northword/zotero-format-metadata) | 50+ 格式 linter、LTWA 期刊缩写 | ✅ 专用工具 **`lint_metadata`**（v2.1.0）：standard/显式规则经 `Zotero.Linter.hooks.onLintInBatch`，未知 id 拒绝 | — |
| [zotero-zotadata](https://github.com/ydeng11/zotero-zotadata) | 多源补全 + 多提供方 PDF 检索 | 经 `run_javascript` 调其检索流水线 | `deep_enrich(itemKey)` 组合补全 + 找 PDF |
